...
generate_gather(int ast)
{
  ...
  int vec[MAXDIMS], vecsec[MAXDIMS], permute[MAXDIMS];
...
generate_hcycliclp(int ast)
{
  ...
  if (normalize_bounds(sptr_lhs)) {
    dim = get_int_cval(A_SPTRG(A_DIMG(ast))) - 1;
    assert(0 <= dim && dim < MAXDIMS, "generate_hcycliclp: bad dim", dim, 4);
                     ...
                     int dim, int std, int astmem)
{
  ...
  assert(dim >= 0 && dim <= MAXDIMS, "inline_hlocalizebnds: illegal dim", dim, 4);
...
pointer_squeezer(int ast)
{
  ...
  case A_SUBSCR:
    ...
    assert(numdim > 0 && numdim <= MAXDIMS, "pointer_squeezer: bad numdim", ast, 4);
    for (i = 0; i < numdim; ++i) {
      int t;
      l = pointer_squeezer((int)ASD_SUBS(asd, i));
      if (l != ASD_SUBS(asd, i))
